display_song_info() {
    ...
    help=$(gum style --padding "1 5" --border double --border-foreground 240 "Pause/Play (p) " "Next Song (n) Previous Song (b)" "Volume+ (j) Volume- (k)" "Check current position (c) Display current queue (d)" "Lyrics (l) Go back (u)" "Kill and go back to menu (t) Silently go back to menu (s)" "Quit (q)")
